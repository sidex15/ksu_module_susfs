...
[ $hide_cusrom = 1 ] && {
	...
	for i in $(find /system /vendor /system_ext /product -iname *lineage* -o -name *crdroid* ) ; do 
		...
		echo "susfs4ksu/boot-completed: [add_sus_path] $i" >> $logfile
	done
}
...
[ $hide_revanced = 1 ] && {
	...
	hide_app () {
		...
		${SUSFS_BIN} add_sus_mount $path && echo "susfs4ksu/boot-completed: [add_sus_mount] $i" >> $logfile
		${SUSFS_BIN} add_try_umount $path 1 && echo "susfs4ksu/boot-completed: [add_try_umount] $i" >> $logfile
		done
	}
